...
if (File.exist?(rdpfile))
  `ruby /home/junm/gaussCode/brlheadmicrobiome/converttaxo_otu_rdp.rb #{qiimefolder}/repr_set.fasta.ignore. #{orimatrixFile}`
else 
...
attrhash=metahash[feature]
  ...
  `ruby ~/gaussCode/brlheadmicrobiome/boxPlotAutoGV-taxDepth.rb #{outDir} #{outDir}/graph/ 6`
  `ruby ~/gaussCode/brlheadmicrobiome/tsvtoxls.rb #{outDir}/RandomForest/ #{feature}`
 ...
 outDir = File.expand_path(settinghash["--outputFolder"]) + "/RF_Boruta/"
   ...
   `ruby ~/gaussCode/brlheadmicrobiome/randomforest_errorrate.rb #{outDir} #{line}`
