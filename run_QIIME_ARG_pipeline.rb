...
basename=File.basename(inputFilePath);
basename.gsub!(/.txt/,"")

outFolder=File.expand_path(settinghash['--outputFolder'])+"/#{basename}/QIIME_result/"
...
cmd << "time pick_otus.py -m #{settinghash["--otuFastMethod"]} -u 0 -i #{seqfile} -o #{pickedOTUsFolder} -s 0.9"
...
# "assign_taxonomy.py -i ./repr_set.fasta -t ~/local/gg_otus_4feb2011/taxonomies/greengenes_tax.txt -r ~/local/gg_otus_4feb2011/rep_set/gg_99_otus_4feb2011.fasta -m blast"
...
if(settinghash["--chimeraSlayerFlag"].to_i==1)
  ...
  cmd << "ChimeraSlayer.pl --query_NAST #{alnFolder}/repr_set_aligned.fasta --exec_dir #{alnFolder}"
...
metanames.each{|meta|
  `ruby ~/gaussCode/brlheadmicrobiome/generate_taxoforitol.rb #{outFolder}/repr_set.fasta.ignore. #{filteredalnFolder} #{filteredalnFolder}/rep_set_aligned.tre #{outFolder}/mapping.txt #{outFolder}/otu_table.txt #{meta}`
}
