...
sampleArray.each{|sample|
  ...
  puts cmdQual="ruby ~/gaussCode/brlheadmicrobiome/run_Blast_Trim_FASTA.rb #{fqFileLocation}  #{outsampleDir} #{sample.barcode} #{sample.sampleName} #{sample.minseqLength} #{sample.minAveQual} #{sample.proximal} #{sample.distal} #{sample.flag1} #{sample.flag2} #{sample.flag3} #{sample.flag4}" 
...
`ruby ~/gaussCode/brlheadmicrobiome/generate_seqmetrics.rb #{outputDir} #{metaline}`
