...
legendlabelfile="#{outdir}/legend_label.txt"
...
metalabelfile="#{outdir}/meta_#{meta}_label.txt"
...
exportpngpath="#{outdir}/export_#{meta}.png"
...
legendlabel=File.open(legendlabelfile,"w")
metalabel=File.open(metalabelfile,"w")
...
classhash.sort_by{|k,v| v.size()}.each{|k,v|
  ...
  v.each{|familyname|
    #puts "\t#{familyname}"
    sample=taxohash[k][familyname][1]
    line="#{sample.classname}: #{sample.ordername}: #{sample.familyname}"
    legendhash[familyname]=line
  }
}
classarray.reverse!
...
mappingline=mappingfile.gets.strip
mapheaders=mappingline.split("\t")
...
`convert -density 450 #{exportpdfpath} #{exportpngpath}`
